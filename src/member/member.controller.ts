...
export class MemberController {
  ...
  ): Promise<TPaging<Member>> {
    const { page, take } = findMemberDto;

    const memberCount: number =
      await this.findMemberService.findMemberCountByConditions(findMemberDto);

    const memberList: Member[] =
      await this.findMemberService.findMemberWithPagingByConditions(
        findMemberDto,
      );
  ...
  async updateMemberById(
    ...
    return await this.updateMemberService.updateMemberById(
      member_id,
      updateMemberDto,
    );
  }
